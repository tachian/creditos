...
- **Rule:** o MVP usa modelo `bridge`: serviços compartilhados com dados e recursos críticos isolados por tenant ou grupo controlado de tenants conforme AD-20. O `tenant_id` confiável vem de autenticação/contexto e do catálogo do `Identity & Tenant`, nunca do body sem validação. `tenant_id` e `tenant_isolation_tier` devem propagar por gRPC metadata, logs, métricas, traces, jobs, filas, cache, storage, callbacks e relatórios; em CloudEvents, o contexto de tenant deve usar extensões válidas (ex.: `tenantid`). Cada serviço deve aplicar enforcement de tenant em autorização, queries, gravações, consumers, producers, cache keys, object keys e dashboards; testes negativos cross-tenant são gate obrigatório antes de produção. Evolução para `silo` ocorre quando risco, volume, contrato, região, performance ou compliance exigirem.
